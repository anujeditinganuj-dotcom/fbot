...
# ytdlp_downloader.py's sites) — faphouse/fpo/porn_fetch don't have one
...
def _guess_site_name_from_link(link: str) -> str:
    ...
    when the backend itself didn't already supply a name (ytdlp_downloader
...
def _display_filename(name: str, title: str | None) -> str:
    """
    Choose the best human-readable filename for the caption's 📄 File Name line.

    Problem: Terabox (and some other hosts) give downloaded files a random
    hash name like "1pdncqsspk3k7wdbyt8ozbg.mkv". The real name lives in
    page_meta["title"] ("1337x-Aurat-Maguva (2022) 720p"). We want to show
    the meaningful title, keeping the original file extension.

    Rules:
      1. No title → return name as-is (safe fallback).
      2. Title available → use title + extension from name.
         e.g. title="Movie Name 720p", name="abc123.mkv" → "Movie Name 720p.mkv"
      3. Title already has the same extension → don't double it.
    """
    if not title:
        return name
    import os as _os
    ext = _os.path.splitext(name)[1]          # ".mkv", ".mp4", …
...
async def build_caption(name: str, size_bytes: int, dl_seconds: float, ul_seconds: float,
    ...
    # ytdlp_downloader.py-backed sites (faphouse.py/fpo.py's get_page_meta
    ...
    # fpo.xxx don't expose them at all — see ytdlp_downloader.py's
    ...
    return (
        f"{title_line}"
        "<blockquote>"
        # FIX: For Terabox (and any site where the downloaded file gets a
        # random/hash name), show the real video title + original extension
        # instead of the meaningless hash. If title is available:
        #   "1pdncqsspk3k7wdbyt8ozbg.mkv"  →  "1337x-Aurat-Maguva (2022).mkv"
        # Falls back to name if no title (direct downloads, cache hits, etc.)
        f"📄 {smallcaps('File Name')}: {smallcaps(_display_filename(name, title))}\n"
        f"{author_line}"
...
class ProgressTracker:
    ...
    def __init__(self, status_msg: Message, label: str, name: str,
        ...
        self._total_bytes  = 0
        self._wait_start   = self.start_time

    def start_upload_wait_animation(self, total_bytes: int = 0):
        """Upload se pehle animated progress bar style dikhata hai jab tak
        Pyrogram file buffer nahi kar leta aur pehla progress callback nahi aata."""
    ...
    async def _upload_wait_loop(self):
        ...
        while not self._first_update:
            ...
            try:
                ...
                size_line = (
                    f"┣⪼ 💾 {human_size(self._total_bytes)}\n"
                    if self._total_bytes else ""
                )
                ...
                await self.status_msg.edit_text(
                    SC(
                        f"📤 <b>Fast Uploading via Main Engine</b>\n\n"
                        "╭━━━━❰Progress❱━➣\n"
                        f"┣⪼ 🎬 File: <code>{self.name}</code>\n"
                        f"{duration_line}"
                        f"{quality_line}"
                        f"┣⪼ [{spin}{'━' * 9}]\n"
                        f"┣⪼ ⚙️ Preparing stream...\n"
                        f"{size_line}"
                        f"┣⪼ 🕐 Elapsed: {human_time(elapsed)}\n"
                        "╰━━━━━━━━━━━━━━━➣\n\n"
                        "⚡ Hyper upload connections active"
                    ),
...
async def autoupload_cmd(client: Client, m: Message):
    ...
    if actor_name:
        ...
        status_msg = await m.reply(
            SC(f"🔍 <b>Looking up \"{actor_name}\" on faphouse...</b>"),
            parse_mode=ParseMode.HTML
        )
        ...
        if not actor_paths:
            ...
            await status_msg.edit_text(SC(
                f"⚠️ <b>Performer \"{actor_name}\" not found.</b>\n\n"
        ...
        await set_chat_scraper_state(target_chat, {"is_running": True})
...
async def jav_cmd(client: Client, m: Message):
    ...
    try:
        ...
        if thumb:
            ...
            await m.reply(text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    except Exception:
        await m.reply(text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
...
async def video_stream_cb(client: Client, query):
    ...
    if pf.is_supported_link(link) or ytdlp.is_supported_link(link):
        # These packages/yt-dlp never expose a raw playable URL (only a
        # format_id/their own download() method) — same limitation as
        # the manual flow's send_stream_link(), and the same reason
        # build_stream_button_markup no longer attaches this button at
        # all for these links. Anyone who still hits this (an older
        # cached button from before that change) gets a clear answer
        # instead of a crash.
        await query.answer(
...
def link_menu_markup(link_id: str, link: str = None) -> InlineKeyboardMarkup:
    ...
    # Same backends build_stream_button_markup() below already excludes —
    # none of pf/ytdlp's packages expose a raw playable URL, only a
    # download() method or a yt-dlp format_id, so a Stream button here
    # would only ever show "not available" when tapped. Hiding it outright
    # is clearer than a button that can never do anything.
    if not (link and (pf.is_supported_link(link) or ytdlp.is_supported_link(link))):
...
def build_stream_button_markup(link: str) -> InlineKeyboardMarkup | None:
    ...
    if pf.is_supported_link(link) or ytdlp.is_supported_link(link) or fpo.is_fpo_link(link):
        return None
...
# FPO.xxx model/performer listing page — same path shape as the generic
# _MODEL_PAGE_RE but scoped to fpo.xxx domains so it doesn't accidentally
# fire for other sites.
_FPO_MODEL_PAGE_RE = re.compile(r"^/models/([a-z0-9-]+)/?$", re.IGNORECASE)


def _is_fpo_model_page(link: str) -> bool:
    """True for fpo.xxx performer listing pages like /models/ellie-nova/."""
    if not fpo.is_fpo_link(link):
        return False
    path = urlparse(link).path
    return bool(_FPO_MODEL_PAGE_RE.match(path))
...
# PornHub search and model browse — cache holds {link_id: {page: [items]}}
BROWSE_CACHE: dict = {}

def _is_pornhub_browse_link(link: str) -> bool:
    """True for pornhub.com model listing pages and /video/search URLs."""
    from urllib.parse import urlparse as _up
    parsed = _up(link)
    host = parsed.netloc.lower()
    if "pornhub.com" not in host:
        return False
    path = parsed.path
    # model / pornstar page
    if _MODEL_PAGE_RE.match(path):
        return True
    # search page  e.g. /video/search?search=comatozze
    if re.search(r"/video/search", path, re.I):
        return True
    return False


def _pornhub_browse_label(link: str) -> str:
    """Human-readable label for a pornhub browse link (for the status message)."""
    from urllib.parse import urlparse as _up, parse_qs as _qs
    parsed = _up(link)
    path   = parsed.path
    m = _MODEL_PAGE_RE.match(path)
    if m:
        return m.group(1).replace("-", " ").replace("_", " ").title()
    # search URL
    qs = _qs(parsed.query)
    q  = (qs.get("search") or qs.get("q") or [""])[0].strip()
    return f'Search: "{q}"' if q else "PornHub Search"
...
def _downloader_for(link: str):
    ...
    see ytdlp_downloader.py's module docstring for why.
    ...
    if terabox.is_terabox_link(link):
        return terabox
    if jav_scraper.is_javct_link(link):
        return jav_scraper
...
async def link_handler(client: Client, m: Message):
    ...
    for i, link in enumerate(links):
        # Admin-only: model/studio listing URL → kick off bulk auto-upload
        if await _maybe_start_actor_bulk_from_link(client, m, link):
            continue
        if await _maybe_start_studio_bulk_from_link(client, m, link):
            continue
        # All users: pornhub model page OR search URL → show browse/pick menu
        if _is_pornhub_browse_link(link):
            await _show_pornhub_browse(client, m, link, page=0)
            continue
        # All users: fpo.xxx performer listing page → show browse/pick menu
        if _is_fpo_model_page(link):
            await _show_fpo_browse(client, m, link, page=0)
            continue
        tag = f"[{i+1}/{len(links)}]" if len(links) > 1 else ""
        await process_link(client, m, link, tag)


BROWSE_PAGE_SIZE = 8   # videos shown per page in browse menu


async def _show_pornhub_browse(client: Client, m: Message, link: str, page: int = 0,
                                status_msg=None):
    """
    Fetch one page of videos from a PornHub model or search URL and show
    them as tappable buttons so the user can pick one to download.

    Works for:
      • https://www.pornhub.com/model/your-priya
      • https://www.pornhub.com/pornstars/mia-khalifa
      • https://www.pornhub.com/video/search?search=comatozze
    """
    from urllib.parse import urlparse as _up
    parsed   = _up(link)
    path     = parsed.path
    label    = _pornhub_browse_label(link)
    link_id  = hashlib.md5(link.encode()).hexdigest()[:12]

    if status_msg is None:
        status_msg = await m.reply(
            SC(f"🔍 <b>Fetching videos for \"{label}\"...</b>"),
            parse_mode=ParseMode.HTML,
        )
    else:
        try:
            await status_msg.edit_text(
                SC(f"🔍 <b>Fetching page {page + 1} for \"{label}\"...</b>"),
                parse_mode=ParseMode.HTML,
            )
        except Exception:
            pass

    # Determine fetch type and call the right scraper
    scraper_page = page + 1   # scraper is 1-indexed, our UI is 0-indexed
    try:
        if re.search(r"/video/search", path, re.I):
            items, _ = await pornhub_scraper.search_videos(link, page=scraper_page)
            browse_type = "search"
        else:
            items, _ = await pornhub_scraper.get_model_page_videos(link, page=scraper_page)
            browse_type = "model"
    except Exception as e:
        logger.warning(f"[phbrowse] fetch failed for {link} p{scraper_page}: {e}")
        items = []
        browse_type = "model"

    if not items:
        await status_msg.edit_text(
            SC(f"😔 <b>No videos found for \"{label}\"</b>"
               + (f" on page {page + 1}." if page > 0 else ".")),
            parse_mode=ParseMode.HTML,
        )
        return

    # Store in cache keyed by link_id (page-keyed sub-dict)
    if len(BROWSE_CACHE) > 200:
        for k in list(BROWSE_CACHE.keys())[:50]:
            BROWSE_CACHE.pop(k, None)
    if link_id not in BROWSE_CACHE:
        BROWSE_CACHE[link_id] = {}
    BROWSE_CACHE[link_id][page] = items
    # Also store metadata needed by callback
    BROWSE_CACHE[link_id]["__meta__"] = {
        "link": link, "label": label, "type": browse_type,
    }

    # Build the inline keyboard — one row per video + prev/next nav
    slice_start = 0
    slice_end   = BROWSE_PAGE_SIZE
    shown = items[slice_start:slice_end]

    buttons = []
    for idx, item in enumerate(shown):
        title = item.get("title") or item.get("slug") or f"Video {idx + 1}"
        title_short = title[:45] + "…" if len(title) > 45 else title
        buttons.append([
            make_button(
                SC(f"🎬 {title_short}"),
                callback_data=f"phpick|{link_id}|{page}|{idx}",
                style=BTN_PRIMARY,
            )
        ])
    ...
    nav_row = []
    if page > 0:
        nav_row.append(make_button(SC("◀️ Prev"), callback_data=f"phbrowse|{link_id}|{page - 1}", style=BTN_PRIMARY))
    if len(items) >= BROWSE_PAGE_SIZE:
        nav_row.append(make_button(SC("▶️ Next"), callback_data=f"phbrowse|{link_id}|{page + 1}", style=BTN_PRIMARY))
    if nav_row:
        buttons.append(nav_row)

    caption = (
        f"🎭 <b>{label}</b>\n"
        f"📄 Page {page + 1}  •  {len(shown)} video(s) shown\n\n"
        "<i>Tap a title to see download quality options:</i>"
    )
    try:
        await status_msg.edit_text(
            SC(caption),
        ...
        logger.warning(f"[phbrowse] edit_text failed: {e}")


async def _show_fpo_browse(client: Client, m: Message, link: str, page: int = 0,
                            status_msg=None):
    """
    Fetch one page of videos from an fpo.xxx performer listing page and
    show them as tappable buttons — same UX as _show_pornhub_browse.

    Works for:
      • https://www.fpo.xxx/models/ellie-nova/
      • https://www.fpo.xxx/models/sarah-arabic/
    """
    from urllib.parse import urlparse as _up
    path   = _up(link).path
    m_obj  = _FPO_MODEL_PAGE_RE.match(path)
    label  = m_obj.group(1).replace("-", " ").title() if m_obj else "FPO Model"
    link_id = hashlib.md5(link.encode()).hexdigest()[:12]
    ...
    scraper_page = page + 1
    try:
        items, total_pages = await asyncio.to_thread(
            fpo.get_model_page_videos, link, scraper_page
        )
    except Exception as e:
        logger.warning(f"[fpobrowse] fetch failed for {link} p{scraper_page}: {e}")
        items, total_pages = [], 1
    ...
    BROWSE_CACHE[link_id][page] = items
    BROWSE_CACHE[link_id]["__meta__"] = {
        "link": link, "label": label, "type": "fpo_model",
        "total_pages": total_pages,
    }

    shown = items[:BROWSE_PAGE_SIZE]
    buttons = []
    for idx, item in enumerate(shown):
        title = item.get("title") or item.get("slug") or f"Video {idx + 1}"
        title_short = title[:45] + "…" if len(title) > 45 else title
        buttons.append([
            make_button(
                SC(f"🎬 {title_short}"),
                callback_data=f"fpopick|{link_id}|{page}|{idx}",
                style=BTN_PRIMARY,
            )
        ])
    ...
    nav_row = []
    if page > 0:
        nav_row.append(make_button(SC("◀️ Prev"), callback_data=f"fpobrowse|{link_id}|{page - 1}", style=BTN_PRIMARY))
    if len(items) >= BROWSE_PAGE_SIZE:
        nav_row.append(make_button(SC("▶️ Next"), callback_data=f"fpobrowse|{link_id}|{page + 1}", style=BTN_PRIMARY))
    if nav_row:
        buttons.append(nav_row)

    caption = (
        f"🎭 <b>{label}</b> <i>(FPO.xxx)</i>\n"
        f"📄 Page {page + 1}  •  {len(shown)} video(s)\n\n"
        "<i>Tap a title to download:</i>"
    )
    try:
        await status_msg.edit_text(
            SC(caption),
        ...
        logger.warning(f"[fpobrowse] edit_text failed: {e}")


@app.on_callback_query(filters.regex(r"^fpobrowse\|"))
async def fpo_browse_page_callback(client: Client, query):
    """Prev/Next navigation for FPO model browse menu."""
    await query.answer()
    parts   = query.data.split("|")
    link_id = parts[1]
    page    = int(parts[2])
    meta = BROWSE_CACHE.get(link_id, {}).get("__meta__")
    if not meta:
        await query.message.edit_text(SC("⚠️ Session expired — please resend the link."), parse_mode=ParseMode.HTML)
        return
    await _show_fpo_browse(client, query.message, meta["link"],
                            page=page, status_msg=query.message)


@app.on_callback_query(filters.regex(r"^fpopick\|"))
async def fpo_browse_pick_callback(client: Client, query):
    """User tapped a video title in the FPO browse menu — go to quality menu."""
    await query.answer()
    parts   = query.data.split("|")
    link_id = parts[1]
    page    = int(parts[2])
    idx     = int(parts[3])
    cached_page = BROWSE_CACHE.get(link_id, {}).get(page, [])
    if not cached_page or idx >= len(cached_page):
        await query.message.edit_text(SC("⚠️ Session expired — please resend the link."), parse_mode=ParseMode.HTML)
        return
    item = cached_page[idx]
    video_url = item.get("url") or item.get("link") or item.get("video_url")
    if not video_url:
        await query.message.edit_text(SC("❌ Could not get video URL for this item."), parse_mode=ParseMode.HTML)
        return
    new_link_id = hashlib.md5(video_url.encode()).hexdigest()[:12]
    LINK_CACHE[new_link_id] = video_url
    await show_quality_menu(client, query, new_link_id, video_url)


@app.on_callback_query(filters.regex(r"^phbrowse\|"))
async def pornhub_browse_page_callback(client: Client, query):
    """Handles Prev/Next page navigation in the PornHub browse menu."""
    await query.answer()
    parts   = query.data.split("|")
    link_id = parts[1]
    page    = int(parts[2])
    ...
    await _show_pornhub_browse(client, query.message, meta["link"],
                                page=page, status_msg=query.message)


@app.on_callback_query(filters.regex(r"^phpick\|"))
async def pornhub_browse_pick_callback(client: Client, query):
    """
    User tapped a video in the PornHub browse list.
    Look up the URL from BROWSE_CACHE and hand it straight to process_link()
    → the same quality-menu flow as if the user had pasted the direct link.
    """
    await query.answer()
    parts    = query.data.split("|")
    link_id  = parts[1]
    ...
    video = cached_page[idx]
    video_url = video.get("url")
    if not video_url:
        await query.message.edit_text(SC("⚠️ Could not get URL for this video."), parse_mode=ParseMode.HTML)
        return

    title = video.get("title") or "Video"
    try:
        await query.message.edit_text(
            SC(f"🔍 <b>Loading qualities for:</b>\n<i>{title[:80]}</i>"),
            parse_mode=ParseMode.HTML,
        )
    except Exception:
        pass

    # Synthesize a minimal Message-like context so process_link works
    await process_link(client, query.message, video_url, tag="")


async def _send_terabox_folder(client: Client, m: Message, link: str, files: list):
    ...
    for idx, f in enumerate(files, start=1):
        ...
        try:
            ...
            caption = await build_caption(
                ...
                user_id=m.from_user.id, source_link=link, quality_label="Original",
            )
            ext = os.path.splitext(final_path)[1].lower()
            if ext in (".mp4", ".mkv", ".mov", ".webm", ".avi"):
                await client.send_video(m.chat.id, final_path, caption=SC(caption), parse_mode=ParseMode.HTML)
            elif ext in (".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"):
                await client.send_photo(m.chat.id, final_path, caption=SC(caption), parse_mode=ParseMode.HTML)
            else:
                await client.send_document(m.chat.id, final_path, caption=SC(caption), parse_mode=ParseMode.HTML)
            sent += 1
        except Exception as e:
...
async def process_link(client: Client, m: Message, link: str, tag: str):
    if terabox.is_terabox_link(link):
        # A real multi-file folder share (2+ resolvable files) skips the
        # normal Download/Stream menu entirely and just sends everything
        # — a single "which quality" menu never made sense for "here's a
        # whole folder" the way it does for one video with several
        # resolutions. A single-file share (or any link this Baidu-PCS
        # path doesn't apply to at all) falls through to the normal menu
        # below unchanged.
        try:
        ...
        if files and len(files) > 1:
            await _send_terabox_folder(client, m, link, files)
            return
...
CALLBACK_HANDLER_FILTER = filters.regex(r"^(dlq|qpage|q|qall|stream|cancel|desc|back)\|")
...
async def show_quality_menu(client: Client, query, link_id: str, link: str, page: int = 0):
                ...
                try:
                    ...
                    await query.message.edit_text(
                        SC(f"<b>No downloadable quality found.</b>\n"
                           ...
                           f"🔗 <a href=\"{link}\">View on javct.net</a>"),
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
                    )
            else:
    ...
    # Terabox-specific metadata fields (Size / Extension / Category)
    # These come from get_page_meta()'s Baidu PCS fallback and are only
    # present for Terabox links — other backends return neither field.
    extra_meta = ""
    if _downloader_for(link) is terabox:
        if page_meta.get("file_size"):
            extra_meta += f"📦 <b>Size:</b> {page_meta['file_size']}\n"
        if page_meta.get("extension"):
            extra_meta += f"📎 <b>Extension:</b> {page_meta['extension']}\n"
        if page_meta.get("category"):
            extra_meta += f"🏷 <b>Category:</b> {page_meta['category']}\n"
        if extra_meta:
            extra_meta = extra_meta.rstrip("\n") + "\n"
...
async def _send_jav_info_card(client: Client, message, link: str, info: dict):
    ...
    # Build /d/ page URL from the video URL (e.g. /v/snos-221-rm → /d/snos-221-rm)
    import re as _re
    _code_m = _re.search(r"/v/([a-z0-9-]+)", link)
    _dl_page_url = f"https://javct.net/d/{_code_m.group(1)}" if _code_m else None
    ...
    if dl_lines:
        ...
        dl_section = (
            f"\n\n<b>🔗 Download Links:</b>\n"
            f"  📥 <a href=\"{_dl_page_url}\">View all download links on javct.net</a>\n"
            f"\n<i>Bot could not fetch links directly (site may be blocking server IPs).\n"
            f"Open the link above to see K2S, RapidGator and other hosts.</i>"
        )
    else:
        dl_section = "\n\n<i>No download links found on this page.</i>"
...
# ===== TITANIUM COMMAND ROUTING =====
# Explicit handlers to route /addbot, /delbot commands to titanium module
# These ensure admin bot properly recognizes titanium commands
@app.on_message(filters.command("addbot") & filters.private, group=0)
async def _route_addbot_to_titanium(client: Client, message: Message):
    """Forward /addbot to titanium handler - handler below will process"""
    pass  # titanium.register_titanium_handlers() will handle this

@app.on_message(filters.command("delbot") & filters.private, group=0)
async def _route_delbot_to_titanium(client: Client, message: Message):
    """Forward /delbot to titanium handler - handler below will process"""
    pass  # titanium.register_titanium_handlers() will handle this
# ===== END TITANIUM COMMAND ROUTING =====
...
async def _download_faphouse_video_inner(client: Client, query, link: str, status_msg, chat_id: int,
    ...
    try:
        ...
        name = f"{slug}.mp4" if slug else f"faphouse_{uuid.uuid4().hex[:8]}.mp4"
        ...
        dl_start = time.time()
        _progress = {"pct": None, "downloaded": 0, "total": 0, "done": False, "error": None,
        ...
        if hasattr(backend_for_meta, "get_page_meta"):
            ...
            async def _tick_meta():
                while True:
                    await asyncio.sleep(3)
                    elapsed = int(time.time() - meta_start)
                    try:
                        await status_msg.edit_text(
                            SC(f"<b>📥 Starting download...</b>\n<code>{name}</code>\n\n"
                               f"<i>Fetching page info... ({elapsed}s)</i>"),
                            parse_mode=ParseMode.HTML,
                        )
                    except Exception:
                        pass

            meta_tick_task = asyncio.create_task(_tick_meta())
            try:
                page_meta = await asyncio.to_thread(backend_for_meta.get_page_meta, link) or {}
            except Exception:
                pass
            finally:
                meta_tick_task.cancel()
        duration_str = _format_duration(page_meta.get("duration")) if page_meta.get("duration") else None

        # Fix: show real video title in progress bar, not the URL-derived slug.
        # For query-string URLs like pornhub.com/view_video.php?viewkey=…
        # urlparse_path_name() returns "view_video.php" which is useless.
        # page_meta["title"] has already been fetched above — use it.
        display_name = page_meta.get("title") or name
        dl_tracker = ProgressTracker(status_msg, "Downloading", display_name, quality=quality_label, duration=duration_str)
        ...
        while not _progress["done"]:
            ...
            pct = _progress["pct"]
            if pct and pct > 2:  # skip the first couple % — early estimates are the noisiest
        ...
        for i, part_path in enumerate(parts, start=1):
            ...
            # Fresh tracker PER PART — each part gets its own start_time/
            # last_edit_time so speed/ETA are computed against that part's
            # own progress, not against elapsed time since part 1 started
            # (old bug: part 2+ showed wrong ETA/speed because it reused
            # part 1's start_time). This also re-arms the "please wait"
            # dots animation for every part, and keeps the tracker's
            # throttled edit_text calls from firing back-to-back with the
            # "Uploading Part i/N..." edit above (FloodWait risk).
            ul_tracker = ProgressTracker(status_msg, "Uploading", name, quality=quality_label, duration=duration_str)
            ul_tracker.start_upload_wait_animation(total_bytes=upload_size)  # progress bar style jab tak callback na aaye
            ...
            part_msg = await client.send_video(
                ...
                # Pyrogram serializes these as raw TL ints — a bare `None`
                # here (which get_video_metadata returns whenever ffprobe
                # can't read the file) crashes deep inside Pyrogram with
                # "'NoneType' object has no attribute 'to_bytes'" instead of
                # a readable error, so always coalesce to 0 (Telegram then
                # just infers dimensions itself).
        ...
        # actually returned one (currently ytdlp_downloader.py's sites;
        ...
        await status_msg.edit_text(SC(f"<b>❌ Unexpected error:</b> {str(e)[:500]}"), parse_mode=ParseMode.HTML)
    finally:
