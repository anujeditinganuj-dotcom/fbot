    ...
    for idx, entry in enumerate(entries, start=1):
        ...
        try:
            ...
            def _blocking_dl(_vu=video_url, _op=out_path):
                try:
                    # FIX: capture return value — yt-dlp sanitizes filenames
                    # (strips special chars, picks correct extension) so the
                    # real file is at the RETURNED path, not the hardcoded
                    # out_path. Previously the return value was discarded and
                    # os.path.exists(out_path) always failed → all 31 videos
                    # downloaded correctly but were skipped on upload.
                    actual = ytdlp.download_video(_vu, _op, on_progress=_on_progress)
                    if actual:
                        _progress["actual_path"] = actual
