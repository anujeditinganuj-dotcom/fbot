    ...
    for idx, entry in enumerate(entries, start=1):
        ...
        try:
            ...
            def _blocking_dl(_vu=video_url, _op=out_path):
                try:
                    # FIX: ytdlp.download_video() returns a (path, elapsed)
                    # TUPLE, not a bare path string (see its signature/return
                    # in ytdlp_downloader.py: `-> tuple[str, float]`). The
                    # previous version here did `actual =
                    # ytdlp.download_video(...)` and stored the whole tuple
                    # as "actual_path" — every video actually downloaded
                    # fine, but os.path.exists() on a tuple raises
                    # TypeError, which the broad except below caught as a
                    # per-video failure, so NOTHING ever got uploaded even
                    # though the downloads themselves were succeeding.
                    #
                    # NOTE: an earlier version of this fix also passed
                    # stream_url="bestvideo[height<=720][ext=mp4]" to cap
                    # quality at ~720p. Reverted for now — untested against
                    # this bot's actual yt-dlp/cache setup, and not worth
                    # risking as a second variable while confirming the
                    # tuple fix itself. Re-add once uploads are confirmed
                    # working again with plain best-quality selection.
            ...
            for p_idx, part_path in enumerate(parts, start=1):
                ...
                await client.send_video(
                    ...
                    progress=lambda cur, tot: asyncio.ensure_future(ul_tracker.update(cur, tot)),
                )
            done_n += 1
        except asyncio.CancelledError:
            shutil.rmtree(work_dir, ignore_errors=True)
            ...
            failed_n += 1
            logger.warning(f"[playlist] {video_url} failed: {e}")
            # Surface the real reason in the Telegram summary too (not just
            # server logs) — up to 3 distinct examples, so a bulk playlist
            # failure is diagnosable from the chat itself.
            if len(failure_samples) < 3:
                failure_samples.append((idx, safe_title, str(e)[:150]))
        finally:
            shutil.rmtree(work_dir, ignore_errors=True)
