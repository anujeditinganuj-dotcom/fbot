...
def _base_opts(url: str = "") -> dict:
    ...
    if url and _is_youtube(url):
        ...
        #
        # PERF FIX: this list had grown to 6 clients (tv, web, ios, mweb,
        # tv_embedded, android) — yt-dlp queries EACH one as its own
        # network round-trip to build the merged format list, so quality
        # fetches were taking 30s+ (confirmed live). "web" alone already
        # gives the FULL ladder once pot_provider is up, so the other
        # clients only need to cover the case where "web" itself fails
        # for a specific video (e.g. some age-gated ones) — one solid
        # fallback is enough for that, not four. Trimmed to 2-3 clients;
        # if a specific video is still missing an expected quality,
        # that's the trade-off to revisit, not something to guess-fix
        # blind here.
