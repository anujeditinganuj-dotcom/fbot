...
def _extract_info(video_url: str) -> dict:
    ...
    try:
        ...
        # DownloadError, not ExtractorError, is what extract_info()
        # actually raises by default (it wraps the underlying
        # ExtractorError) — catching only ExtractorError here meant this
        # whole fallback chain (embed-URL retry, then direct HTML scrape)
        # never even ran; the exception passed straight through past this
        # except clause, all the way out to download_video()'s own outer
        # try/except, which happens to log under the same "[eporner-
        # direct]" prefix — misleadingly making it look like the direct-
        # scrape fallback itself had been tried and failed, when it was
        # never reached at all. Catching both exception types (whichever
        # a given yt-dlp version/config actually raises) is what makes
        # the fallback logic below actually run.
        err = str(e)
