...
def _fetch_latest_items_sync() -> list:
    ...
    for url in _API_LATEST_URLS:
        try:
            ...
            if items:
                logger.debug(f"[xhamster] latest via JSON API: {len(items)} items from {url}")
                return items
        except Exception as e:
            logger.debug(f"[xhamster] JSON API failed {url}: {e}")
    ...
    for url in _HTML_LATEST_URLS:
        try:
            ...
            if items:
                logger.debug(f"[xhamster] latest via HTML scrape: {len(items)} items from {url}")
                return items
        except Exception as e:
            logger.debug(f"[xhamster] HTML scrape failed {url}: {e}")

    # Strategy 3: yt-dlp with /newest only (avoid /best/all and /videos)
    # /best/all → yt-dlp generic extractor tries "all" as video ID → 404
    # /videos   → yt-dlp generic extractor tries "videos" as video ID → 404
