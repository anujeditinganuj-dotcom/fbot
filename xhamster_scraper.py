...
FIX v3:
  ...
  + /videos/recent, /best/all, /newest all 404 on Render IPs — API used instead
...
def _fetch_latest_items_sync() -> list:
    ...
    # Strategy 3: yt-dlp (often blocked on Render IPs but try anyway)
    for domain in _DOMAINS:
        for path in ["/videos", "/best/all"]:
            url = domain + path
            try:
