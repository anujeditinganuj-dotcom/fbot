        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                if page == 1 and not videos:
                    # Page 1 empty = model not found or wrong slug
                    slug_used = re.sub(r"[^a-z0-9-]", "-", model_name.lower().strip()).strip("-")
                    msg = (
                        f"❌ <b>FPO model not found:</b> <code>{model_name}</code>\n"
                        f"Tried URL: <code>fpo.xxx/models/{slug_used}/</code>\n\n"
                        f"✅ Sahi slug use karo — FPO model page ka URL open karo "
                        f"aur slug copy karo (e.g. <code>sarah-arabic-2</code>)."
                    )
                else:
                    msg = (
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
