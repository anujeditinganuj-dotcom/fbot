        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                if page == 1 and not videos:
                    ...
                    # search_model_slug() already tried alternate slugs
                    # and the FPO search endpoint — this is a genuine miss.
                    slug_used = re.sub(r"[^a-z0-9-]", "-", model_name.lower().strip()).strip("-")
                    search_q = model_name.strip().replace(" ", "+")
                    msg = (
                        f"❌ <b>FPO model not found:</b> <code>{model_name}</code>\n"
                        f"Tried slug: <code>/models/{slug_used}/</code> + search fallbacks.\n\n"
                        f"💡 <b>Options:</b>\n"
                        f"• Search manually: <a href=\"https://www.fpo.xxx/search/models/?q={search_q}\">fpo.xxx search</a>\n"
                        f"• Paste her exact URL:\n"
                        f"  <code>/autouploadfpo https://www.fpo.xxx/models/SLUG/</code>\n\n"
                        f"<i>She may not be on FPO.xxx, or her slug is different from her name.</i>"
                    )
                else:
                    msg = (
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
