...
from database import (
    is_video_uploaded, save_uploaded_video, get_uploaded_video_count,
...
async def get_page_videos(page: int = 1, base_url: str = None, path: str = "/videos") -> list:
    ...
    try:
        ...
        logger.warning(f"[scraper] {base_url}{path} page {page} fetch failed: {e}")
        return []
...
async def discover_actor_paths(name: str) -> dict:
    ...
    for base_url in SCRAPE_SITES:
        # ── Pattern probing ──
        for pattern in ACTOR_PATH_PATTERNS:
...
async def discover_category_paths(tag_name: str) -> dict:
    ...
    found = {}
    for base_url in SCRAPE_SITES:
        for pattern in CATEGORY_PATH_PATTERNS:
            path = pattern.format(slug=slug)
            try:
                items = await get_page_videos(page=1, base_url=base_url, path=path)
            except Exception as e:
...
async def discover_studio_paths(name: str) -> dict:
    ...
    for base_url in SCRAPE_SITES:
        for candidate_slug in slug_candidates:
            for pattern in STUDIO_PATH_PATTERNS:
...
async def process_and_upload_video(client: Client, video_url: str, chat_id: int,
    ...
    try:
        ...
        stream_markup = _build_stream_markup(video_url) if _build_stream_markup else None
        ...
        if len(parts) == 1:
            ...
            async def _part_progress(current, total_bytes):
                # Reuses the same single progress bubble for every part —
                # the tracker itself only cares about (current, total), so
                # relabeling which part is in flight happens in the
                # "Starting upload — Part i/N" edit right before each
                # send_video call below, not inside this callback.
                if ul_tracker:
                    await ul_tracker.update(current, total_bytes)

            sent_msgs = []
            for i, part_path in enumerate(parts, start=1):
                # ── Part status edit (no tracker running yet → no conflict) ──
                if progress_msg:
                    try:
                        await progress_msg.edit_text(
                ...
                # Bug fix: reusing the same ul_tracker across parts meant
                # Part 2+ had a stale last_edit_time from Part 1's end →
                # the throttle check suppressed all progress updates for
                # Part 2 (and beyond), making those uploads look frozen.
                # A new tracker resets start_time and last_edit_time so
                # each part's progress bar starts clean.
    ...
    try:
        while True:
            ...
            if not videos:
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"✅ <b>Auto-upload complete.</b>\n📊 Total uploaded: {state.get('total_scraped', 0)}",
                    )
                except Exception:
                    pass
                break
            ...
            for item in videos:
                ...
                if ok:
                    total = state.get("total_scraped", 0) + 1
                    state["total_scraped"] = total
                    await set_chat_scraper_state(chat_id, {"total_scraped": total})
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                else:
                    # Transient failure (process_and_upload_video's own
                    # contract: False means "retry later", not "skip
                    # forever"). Back off harder than the normal
                    # between-video cooldown instead of immediately
                    # hammering the site/Telegram again — a burst of
                    # failures is often a sign of a rate limit or expired
                    # session, and retrying at full speed only makes that
                    # worse.
    ...
    try:
        if not model_name:
            while True:
                ...
                for item in videos:
                    ...
                    if ok:
                        total = state.get("eporner_total_scraped", 0) + 1
                        await set_chat_scraper_state(chat_id, {"eporner_total_scraped": total})
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                    else:
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)

                if not any_new:
                    # Every video in this random batch was already seen —
                    # short pause before rolling a fresh random page/order
                    # rather than hammering the API back-to-back.
                    await asyncio.sleep(5)
            return
        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                try:
                    await client.send_message(
                        ...
                        f"📊 Total uploaded: {state.get('eporner_total_scraped', 0)}",
                    )
                except Exception:
                    pass
                break
            ...
            for item in videos:
                ...
                if ok:
                    total = state.get("eporner_total_scraped", 0) + 1
                    state["eporner_total_scraped"] = total
                    await set_chat_scraper_state(chat_id, {"eporner_total_scraped": total})
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                else:
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
    ...
    try:
        if not model_name:
            while True:
                ...
                for item in videos:
                    ...
                    if ok:
                        total = state.get("pornhub_total_scraped", 0) + 1
                        await set_chat_scraper_state(chat_id, {"pornhub_total_scraped": total})
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                    else:
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
                        f"📊 Total uploaded: {state.get('pornhub_total_scraped', 0)}",
                    )
                except Exception:
                    pass
                break
            ...
            for item in videos:
                ...
                if ok:
                    total = state.get("pornhub_total_scraped", 0) + 1
                    state["pornhub_total_scraped"] = total
                    await set_chat_scraper_state(chat_id, {"pornhub_total_scraped": total})
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                else:
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
    ...
    try:
        if not model_name:
            while True:
                ...
                for item in videos:
                    ...
                    if ok:
                        total = state.get("xhamster_total_scraped", 0) + 1
                        await set_chat_scraper_state(chat_id, {"xhamster_total_scraped": total})
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                    else:
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
                        f"📊 Total uploaded: {state.get('xhamster_total_scraped', 0)}",
                    )
                except Exception:
                    pass
                break
            ...
            for item in videos:
                ...
                if ok:
                    total = state.get("xhamster_total_scraped", 0) + 1
                    state["xhamster_total_scraped"] = total
                    await set_chat_scraper_state(chat_id, {"xhamster_total_scraped": total})
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                else:
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
    ...
    try:
        if not model_name:
            while True:
                ...
                for item in videos:
                    ...
                    if ok:
                        total = state.get("xvideos_total_scraped", 0) + 1
                        await set_chat_scraper_state(chat_id, {"xvideos_total_scraped": total})
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                    else:
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
        ...
        while True:
            ...
            if not videos or page > total_pages:
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
                        f"📊 Total uploaded: {state.get('xvideos_total_scraped', 0)}",
                    )
                except Exception:
                    pass
                break
            ...
            for item in videos:
                ...
                if ok:
                    total = state.get("xvideos_total_scraped", 0) + 1
                    state["xvideos_total_scraped"] = total
                    await set_chat_scraper_state(chat_id, {"xvideos_total_scraped": total})
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                else:
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
        ...
        if not model_name:
            ...
            while True:
                ...
                for item in videos:
                    ...
                    if ok:
                        total = state.get("fpo_total_scraped", 0) + 1
                        await set_chat_scraper_state(chat_id, {"fpo_total_scraped": total})
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN)
                    else:
                        await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
            ...
            if not videos or page > total_pages:
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"✅ <b>Auto-upload complete for \"{model_name}\".</b>\n"
                        f"📊 Total uploaded: {state.get('fpo_total_scraped', 0)}",
                        parse_mode=ParseMode.HTML,
                    )
                except Exception:
                    pass
        ...
        while True:
            ...
            for item in videos:
                ...
                if ok:
                    ...
                    # Transient failure — see category_uploader_worker's
                    # comment above for why this backs off harder instead
                    # of hammering the site/Telegram again immediately.
                    await asyncio.sleep(2 if is_admin else AUTO_UPLOAD_COOLDOWN * 3)
...
async def xhamster_live_monitor(client: Client, default_channel: int):
    """xhamster.com's equivalent of eporner_live_monitor — identical
    shape (same pause-while-a-manual-job-owns-the-channel guard, same
    _process_with_retries plumbing), just sourced from
    xhamster_scraper.get_latest_videos() (page 1 of xHamster's own
    /newest listing — see that function's docstring) instead of
    eporner's rotating-search approximation, and downloaded via
    ytdlp_downloader (backend=ytdlp) same as eporner_live_monitor.

    Uncapped, same as live_site_monitor and eporner_live_monitor (see
    that one's docstring for why it was capped originally and why that
    tradeoff was deliberately removed here too)."""
...
async def xvideos_live_monitor(client: Client, default_channel: int):
    """xvideos.com's equivalent of xhamster_live_monitor — identical
    shape (same pause-while-a-manual-job-owns-the-channel guard, same
    _process_with_retries plumbing, uncapped same as the other two
    monitors), just sourced from xvideos_scraper.get_latest_videos()
    (page 1 of XVideos' own /new listing) instead of xHamster's, and
    downloaded via ytdlp_downloader (backend=ytdlp) same as the others."""
