        ...
        if on_progress:
            ...
            if m:
                ...
                on_progress({
                    ...
                    "eta_s": _size_to_bytes(eta) if eta else None,
                    "elapsed_s": elapsed,
                    "duration_s": 0,
                })
    process.wait()
...
def download_video(video_url: str, out_path: str, on_progress=None, stream_url: str = None) -> tuple[str, float]:
    ...
    # BUG FIX: NamedTemporaryFile with delete=False created BEFORE the try block
    # meant that if an exception occurred between file creation and the try,
    # the file would leak. Now the file is created inside the try/finally so
    # cleanup is always guaranteed.
    cookie_file = None
    cookies = _parse_cookie_string(FPO_COOKIES)
    if cookies:
    ...
    ydl_opts = {
        ...
        "retries": 5,
        "fragment_retries": 10,
        # Only set http_headers when NOT using impersonate — when impersonate
        # is active, curl_cffi sends a full consistent Chrome header set
        # automatically. Overriding User-Agent alone while leaving the rest
        # as curl_cffi defaults creates a detectable fingerprint mismatch.
        **({"http_headers": {"User-Agent": _UA, "Referer": DEFAULT_BASE_URL}}
           if not imp_ok else {"http_headers": {"Referer": DEFAULT_BASE_URL}}),
...
def get_model_page_videos(model_name: str, page: int = 1) -> tuple[list[dict], int]:
    ...
    # Page 1 empty → slug might be wrong, try search as fallback
    if page == 1 and not items and not model_name.startswith("http"):
        logger.info(f"[fpo] slug {slug!r} returned empty — searching for correct slug")
