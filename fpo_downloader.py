...
_VIDEO_LISTING_RE = re.compile(
    # FIX: fpo.xxx ne href="..." se href='...' switch kiya — old pattern sirf
    # double quotes match karta tha. Backreference \1 ensures opening/closing
    # quote match kare. Groups: (1)=quote, (2)=path, (3)=video_id
    r"""href=(["'])(/video/(\d+)/[^"']+)\1""",
    re.IGNORECASE,
)
...
def get_model_page_videos(model_name: str, page: int = 1) -> tuple[list[dict], int]:
    ...
    fpo.xxx URL (used as-is, with ?page= appended).
    ...
    if model_name.startswith("http"):
        ...
        url = _search_url(model_name, page)
