...
def search_model_slug(name: str) -> str | None:
    ...
    for search_url in [
        ...
        try:
            ...
            # Look for /models/<slug>/ links in the search results
            slugs = re.findall(r'href=["\'/]+models/([a-z0-9-]+)/?', html)
            if slugs:
                logger.info(f"[fpo] model search for {name!r} found slug: {slugs[0]!r}")
                return slugs[0]
        except Exception as e:
