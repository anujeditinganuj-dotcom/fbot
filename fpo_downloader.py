...
def search_model_slug(name: str) -> str | None:
    ...
    # fpo.xxx slug regex — matches /models/<slug>/ in any URL context
    _SLUG_RE = re.compile(r'/models/([a-z0-9][a-z0-9-]*[a-z0-9])/?', re.IGNORECASE)

    for search_url in [
        f"{DEFAULT_BASE_URL}/search/models/?q={query}",
        f"{DEFAULT_BASE_URL}/models/?q={query}",
        f"{DEFAULT_BASE_URL}/search/?q={query}&type=models",
        # Also try without URL-encoding (some names need literal spaces)
        f"{DEFAULT_BASE_URL}/search/models/?q={name.strip().replace(' ', '%20')}",
    ]:
        try:
            html = _fetch_html(search_url)
            slugs = _SLUG_RE.findall(html)
            # Filter out generic/nav slugs that appear on every page
            filtered = [s for s in slugs if s not in ("models", "search", "categories", "tags")]
            if filtered:
                logger.info(f"[fpo] model search for {name!r} found slug: {filtered[0]!r} (via {search_url})")
                return filtered[0]
        except Exception as e:
