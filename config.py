...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8677080919:AAEc3pUbXrKyDhDG7aw7snp20TIp-2XXYaM")
...
# PO-token support (YOUTUBE_POT_ENABLED / bgutil-ytdlp-pot-provider) was
# removed entirely — it forced yt-dlp's "web" YouTube client, which
# without a real working PO token (the provider was never actually wired
# into yt-dlp's requests — pot_helper.py didn't exist) got served only
# 1-2 formats by YouTube instead of the full ladder. ytdlp_downloader.py
# now always uses ios/mweb/tv_embedded/android, which don't need a PO
# token and return their own real (if smaller) format lists reliably.
